  ...
  const handleBuy = async (product: Product) => {
    ...
    try {
      ...
      if (data.url) {
        ...
        alert('Checkout session initialization failure.');
      }
    } catch (e) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {products.map((p) => (
            <div key={p.id} onClick={() => handleBuy(p)} style={{ backgroundColor: '#181b26', padding: '12px', borderRadius: '6px', border: '1px solid #0284c7', cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: '0.8rem', color: '#38bdf8' }}>{p.category} • {p.price} • {p.sales} Sold — <span style={{ textDecoration: 'underline' }}>{loadingId === p.id ? 'Loading Checkout...' : 'Buy Now ⚡'}</span></div>
            </div>
          ))}
        </div>
      </div>
